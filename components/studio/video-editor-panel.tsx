...
export function VideoEditorPanel({ project }: VideoEditorPanelProps) {
  ...
  // Default OFF — keeps the exported MP4 clean. Users who want burned-in
  // captions can flip it on; the separate SRT file is still produced and
  ...
  // Quick fade-in intro, not a standalone scene. ~0.9s is the default fade
  // length; users can extend up to ~3s if they want a longer splash.
  const [coverDurationSec, setCoverDurationSec] = useState(0.9);
  ...
  // All "final" video renders — excludes per-scene AI video clips and the
  // short vertical clips generated by the Shorts tool. Sorted newest-first
  // so the dropdown shows the latest export at the top.
  const finalVideoAssets = useMemo(() => {
    const list = (project?.assets ?? []).filter((a) => {
      if (a.type !== "video") return false;
      const kind =
        (a.metadataJson as { kind?: string } | null)?.kind ?? "final";
      return kind !== "scene-clip" && kind !== "short";
    });
    return list.sort(
      (a, b) =>
        new Date(b.createdAt as string).getTime() -
  ...
  const finalVideoAsset = finalVideoAssets[0];
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-slate-950 text-sm">
      ...
      <div className="flex items-center justify-between border-b border-white/10 bg-slate-900/80 px-4 py-2">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Button flips between three states:
              - Rendering → progress label (disabled)
              - Has a completed render → Download MP4 (primary) + Re-export (secondary)
              - Nothing rendered yet → Export MP4 (primary) */}
          {rendering ? (
            <Button disabled className="min-w-[170px]">
              {renderStatus === "queued" ? "Queued…" : `Rendering ${renderProgress}%`}
            </Button>
          ) : finalVideoAsset ? (
            <div className="flex items-center gap-2">
              <DownloadHistoryButton renders={finalVideoAssets} />
              <Button
                variant="outline"
                onClick={handleRender}
                disabled={!everySceneHasImage || !audioAsset}
        ...
        <aside className="flex w-64 shrink-0 flex-col border-r border-white/10 bg-slate-900/50">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-3">
            ...
            <div>
              ...
              {/* Clicking opens the right-side properties panel which has
                  BOTH the AI-generate and Upload-from-device actions. Users
                  can also drop a file directly on the tile for the fast path. */}
              <button
                type="button"
              ...
              >
                {coverAsset ? (
                  ...
                  <div className="flex aspect-video flex-col items-center justify-center gap-1 bg-black/30 px-2 text-center text-[10px] text-muted-foreground">
                    <span className="font-semibold text-slate-300">Click to set a cover</span>
                    <span>Generate with AI or upload from device</span>
                  </div>
                )}
                {coverDropActive ? (
                  <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-amber-500/30 text-[10px] font-semibold text-white">
              ...
              <div className="space-y-3">
                ...
                {coverAsset ? (
                  <div className="rounded-xl border border-cyan-400/20 bg-cyan-500/5 p-2 text-[11px] text-cyan-200">
                    ✓ Cover will fade into scene 1 over the first{" "}
                    {coverDurationSec.toFixed(1)}s of your next render. Toggle
                    under <em>Export settings</em> → &quot;Add cover as intro&quot;.
                  </div>
                ) : null}
                <p className="text-[11px] text-muted-foreground">
...
"Vintage oil painting, moody brushstrokes"`}
                ...
                {finalVideoAsset ? (
                  <>
                    ...
                    />
                    <p className="text-[11px] text-muted-foreground">
                      To export WITHOUT subtitles, untick <em>&quot;Burn subtitles
                      into MP4&quot;</em> above and click <em>Export MP4</em> — that render
                      has no burned subs. The SRT file is always downloadable
                      separately from the Narration panel.
                    </p>
...
/**
 * Top-bar Download button with a full history of every render this project
 * has produced. Each entry shows the date/time + kind (if known) so the
 * user can pick the exact export they want after multiple re-exports.
 */
function DownloadHistoryButton({
  ...
  return (
    <div ref={wrapperRef} className="relative">
      <div className="flex items-center gap-1">
        {/* Primary click = download the most recent render. */}
        <Button asChild className="min-w-[140px]">
          <a href={`/api/assets/${latest.id}?download=1`}>
            <Download className="mr-2 h-4 w-4" /> Download MP4
          </a>
        </Button>
        {/* Dropdown toggle = pick an older export. */}
        {renders.length > 1 ? (
          <Button
            variant="outline"
            size="sm"
            onClick={() => setOpen((v) => !v)}
            title={`${renders.length} exports`}
          >
            ▾
          </Button>
        ) : null}
      ...
      {open ? (
        <div className="absolute right-0 top-full z-30 mt-1 w-80 overflow-hidden rounded-xl border border-white/10 bg-slate-900 shadow-2xl">
          <p className="border-b border-white/10 bg-white/5 px-3 py-2 text-[11px] uppercase tracking-wider text-muted-foreground">
            Your exports ({renders.length})
          </p>
          <div className="max-h-80 overflow-y-auto p-1">
            {renders.map((r, i) => {
              ...
              return (
                <a
                  key={r.id}
                  href={`/api/assets/${r.id}?download=1`}
                  onClick={() => setOpen(false)}
                  className={`flex items-center justify-between rounded-lg px-3 py-2 text-xs hover:bg-white/10 ${
                    i === 0 ? "bg-white/5" : ""
                  }`}
                >
                  <div className="min-w-0">
                    <p className="font-medium">
                      {i === 0 ? "Latest export" : `Export #${renders.length - i}`}
                      {aspect ? (
                        <span className="ml-2 rounded bg-cyan-500/20 px-1.5 text-[10px] text-cyan-200">
                          {aspect}
                        </span>
                      ) : null}
                      {kind !== "final" ? (
                        <span className="ml-1 rounded bg-white/10 px-1.5 text-[10px] text-muted-foreground">
                          {kind}
                        </span>
                      ) : null}
                    </p>
                  ...
                  <Download className="h-3 w-3 shrink-0 text-cyan-300" />
                </a>
              );
            })}
          </div>
...
/**
 * Download dropdown — shows 1080p (source) plus on-demand transcodes to
 * 720p / 480p / 360p. Transcode happens on the server via FFmpeg scale;
 * repeat clicks at the same height hit the cached variant.
 */
function DownloadMenu({
  ...
  const [open, setOpen] = useState(false);
  const [busyHeight, setBusyHeight] = useState<number | null>(null);
  const [error, setError] = useState<string | null>(null);

  async function downloadAt(height: number) {
    setBusyHeight(height);
    setError(null);
    try {
      const res = await fetch(`/api/projects/${projectId}/transcode`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ assetId, height })
      });
      const payload = await res.json();
      if (!res.ok) throw new Error(payload.error ?? "Transcode failed");
      // Kick off the download by navigating the browser to the signed URL.
    ...
    <div className="relative space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <Button asChild size="sm">
          <a href={`/api/assets/${assetId}?download=1`}>
      ...
      {open ? (
        <div className="grid grid-cols-3 gap-1.5 rounded-xl border border-white/10 bg-black/30 p-2">
          {[1080, 720, 480, 360].map((h) => (
            <button
              key={h}
              type="button"
              disabled={busyHeight !== null}
              onClick={() => void downloadAt(h)}
              className="rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-xs hover:bg-white/10 disabled:opacity-40"
            >
              {busyHeight === h ? "…" : `${h}p`}
            </button>
          ))}
...
function TimelineTrack({
  ...
  return (
    <div className="relative flex h-12 items-center">
      <div
        title={label}
        className="sticky left-0 z-[5] mr-1 flex h-full w-28 shrink-0 items-center overflow-hidden rounded-l-lg border border-white/5 bg-slate-900 px-2 text-[10px] uppercase tracking-wider text-muted-foreground"
      >
